...
public class ActionButtonItems extends View implements ValueAnimator.AnimatorUpdateListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBitmap != null) {
            ...
            canvas.drawText("原位实验", mRect.exactCenterX() - 300, mRect.exactCenterY(), paint);
        }
        super.onDraw(canvas);
    }
...
//                    ((ActionMenu) getParent()).openMenu();
                    ((ActionMenu) getParent()).setOpen(true);
                } else if ((isSwitchButton && isOpen) || !isSwitchButton) {
                    ((ActionMenu) getParent()).setOpen(false);
//                    ((ActionMenu)getParent()).closeMenu();
                }
                if (itemClickListener != null) {
                    itemClickListener.onItemClick(tag, this, isOpen);
                }
                isPress = false;
                return true;
        }
